...
<form method="post" action="<?php esc_url( $_SERVER['REQUEST_URI'] ) ?>">
	...
	<div class="form-group">
		<label for="password"><?php _e('Password' , $this->get_language_domain() ) ?></label>
	...
	<button type="submit" class="btn btn-success btn-custom" name="<?php echo $this->get_prefix() ?>submit-login"><?php _e( 'Login' , $this->get_language_domain()) ?></button>
	
	<p><a href="/fidelity-card-recovery"><?php _e( 'Hai perso la password?' , $this->get_language_domain() ); ?></a></p>
